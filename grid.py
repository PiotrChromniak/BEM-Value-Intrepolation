...
def generate_grid_points(ranges, grid_density, grid_borders):
    ...
    for i in range(grid_density):
        ...
        for j in range(y_density):
            ...
            valid_ranges = [
                f_range for f_range in ranges if f_range.is_in_range(current_x)]
            intersection_counters = [f_range.get_intersections_count(
                current_x, current_y) for f_range in valid_ranges]
            intersection_count = sum(intersection_counters)
...
for point in grid_points:
    print('r: {:.2}'.format(math.sqrt(point[0]**2 + point[1]**2)))
